...
namespace Twidibot2
{
	public class DB
	{
		...
		public DB(Twidibot.TechFuncs techFuncs, string DBName) {
			this.TechFuncs = techFuncs;
			this.DBName = DBName;
			SQLCon = new SQLiteConnection("Data Source=" + this.DBName);
			//Query = new SQLiteCommand(SQLCon);
		...
		public class SettingsTC {
			...
			public bool UpdateSetting(string SettingName, string Parameter) {
				...
				if (Setl != null) {
					if (Parameter == null) {
						sql = "UPDATE Settings SET Param = null WHERE SetName='" + SettingName + "';";
					} else {
		...
		public class DefCommandsTC {
			...
			public bool Remove(int id) {
				...
				if (val == 1) {
					...
					db.TechFuncs.LogDH("Удалена команда \"" + DefComl.Command + "\"");
					return true;
				} else {
					return false;
				}
			...
			public bool UpdateLastUsed(int id, string DateTime) {
				...
				DefCommand_tclass DefComl = db.DefCommandsList.ElementAt(i);
				if (DefComl != null) {
					sql = "UPDATE DefCommands SET LastUsed='" + DateTime + "' WHERE id='" + id + "';";
					Q.CommandText = sql;
					Q.ExecuteNonQuery();
		...
		public class FuncCommandsTC {
			...
			public bool UpdateLastUsed(int id, string DateTime) {
				...
				if (FuncComl != null) {
					sql = "UPDATE FuncCommands SET LastUsed='" + DateTime + "' WHERE id='" + id + "';";
					Q.CommandText = sql;
					Q.ExecuteNonQuery();
		...
		public class SpamMessagesTC {
			...
			public SpamMessage_tclass Find(string Message) {
				...
				Q.CommandText = "SELECT * FROM SpamMessages WHERE Message = '" + Message + ";";
				R = Q.ExecuteReader();
				if (R.HasRows) {
					R.Read();
					sout.id = R.GetInt32(0);
			...
			public bool UpdateLastUsed(int id, string DateTime) {
				...
				SpamMessage_tclass SpamMsgl = db.SpamMessagesList.ElementAt(i);
				if (SpamMsgl != null) {
					sql = "UPDATE SpamMessages SET LastUsed='" + DateTime + "' WHERE id='" + id + "';";
					Q.CommandText = sql;
					Q.ExecuteNonQuery();
		...
		public class Funcs_TimersTC {
			...
			public int Add(Funcs_Timer_tclass Timerl) {
				...
				SQLiteCommand Q = new SQLiteCommand(db.SQLCon);
				SQLiteDataReader R = null;
				sql = "INSERT INTO Funcs_Timers (Name, Time, Nick, DTStart, ChannelNotif, Paused) VALUES ('" + Timerl.Name + "', '" + Timerl.Time + "', '" + Timerl.Nick + "', '" + Timerl.DTStart + "', '" + Convert.ToInt32(Timerl.ChannelNotif) + "', '" + Convert.ToInt32(Timerl.Paused) + "'); SELECT last_insert_rowid();";
				Q.CommandText = sql;
				R = Q.ExecuteReader();
				R.Read();
			...
			public bool PauseUpdate(string Name, bool Paused) {
				...
				if (Timerl != null) {
					...
					if (Timerl.Paused != Paused) {
						if (Paused) {
							sql = "UPDATE Funcs_Timers SET DTPauseResume = '" + dtprorg + DTstr + ",', Paused = '1' WHERE Name = '" + Name + "';";
						} else {
							sql = "UPDATE Funcs_Timers SET DTPauseResume = '" + dtprorg + DTstr + ";', Paused = '0' WHERE Name = '" + Name + "';";
						}
			...
			public bool UpdateTime(string Name, ulong Time) {
				...
				int i = db.TimersList.FindIndex(x => x.Name == Name);
				Funcs_Timer_tclass Timerl = db.TimersList.ElementAt(i);
				sql = "UPDATE Funcs_Timers SET Time = '" + Time.ToString() + "' WHERE Name = '" + Name + "';";
				Q.CommandText = sql;
				int val = Q.ExecuteNonQuery();
				if (val == 1) {
					db.TimersList.RemoveAt(i);
		...
		*/
		...
		public SQLiteDataReader SQLQuery(string SQL, bool DataReceived = false) {
